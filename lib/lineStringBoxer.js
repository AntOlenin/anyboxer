...
function getBoxes(lineString, options, callback) {
    ...
    filterIntersectBoxes(subBoxes, coordinates, function(err, result) {
        var necessaryBoxes = getNecessaryBoxes(result, subBoxes);
        callback(null, mergeBoxes(necessaryBoxes));
    });
}
...
function filterIntersectBoxes(boxes, coordinates, callback) {
    return async.map(boxes,

        function(oneLineSubBoxes, cb) {
            var result = _.filter(oneLineSubBoxes, function(box) {
